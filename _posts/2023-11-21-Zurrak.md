...
# 01. Wstęp
[HackMyVM](https://hackmyvm.eu/machines/machine.php?vm=Zurrak) jest już drugą maszynką od [tasiyanci](https://hackmyvm.eu/profile/?user=tasiyanci). Złamanie jej zajmuje trochę czasu, zwłaszcza, jeżeli nie miałeś wcześniej do czynienia z narzędziami do SMB. Okazało się, że część nie działa tutaj, tak jak trzeba (o tym później (a może ja nie działałem jak trzeba ;)). Obraz ma też możliwych rozwiązań. Nie wiem, czy to było w planie autora czy je przypadkowo znalazłem. Szczerze polecam.
{: .text-justify}
# 02. Wstęp
Po uzyskaniu ip maszyny, najczęściej następuje skanowanko. Użyję komendę __db_nmap__ z **Metasploita**
{: .text-justify}
```bash
...
```
Widać otwarte porty: **80/tcp** - Apache WWW, **445/tcp** -Samba smbd i **5432/tcp** - PostgreSQL
# 03. Strona www
...
{: .text-justify}
Niestety, widać tylko początek strony
{: .text-justify}
...
Wpisując _admin.php_ przenosi nas na _index.php_
{: .text-justify}
## 03a. Burp Suite i obrazki (bez łamania tokena)
Przechwytujemy stronę przez Burp Suita i widzimy **token**, ale teraz jest nieistotny. Wrzucamy to wszystko do **Repeater**a. Tak jak się spodziewaliśmy, dalej jest tylko **Welcome**
{: .text-justify}
...
```
I dodanie dosłownie __byle czego__, dostajemy takie coś.
{: .text-justify}:
![06](/assets/images/hacking/2023/04/06.png)
A tam na stronie mamy informację, że udział **SMB** nazywa się **share** i mamy obrazki: **zurrakhorse.jpg**, **zurraksnake.jpg** i **zurrakhearts.jpg**
{: .text-justify}:
...
Wchodząc na [stronę](https://jwt.io/) wrzucamy token i widzimy takie coś
{: .text-justify}:
![08](/assets/images/hacking/2023/04/08.png)
Należy zwrócić uwagę na to, że pole z adminem wygląda tak: __"isAdmin": false__,. Spróbujmy więc złamać ten token, uzyskać hasło i zmienić __"isAdmin":__ na __true__. Do tego bardzo dobrze nadaje się **Hashcat**:
{: .text-justify}:
```bash
.\hashcat.exe -O -a0 .\hashe\zurrak.txt .\dict\rockyou.txt
```
![09](/assets/images/hacking/2023/04/09.png)
**Hashcat** automatycznie znalazł jaki to jest hash i go odkodował. Hasło to **TEST123**
{: .text-justify}:
Wpisujemy hasło, zmieniamy __"isAdmin"__ na __true__
{: .text-justify}:
![10](/assets/images/hacking/2023/04/10.png)
{: .text-justify}:
Po wrzuceniu tokenu mamy taki sam wynik, jak we wcześniejszym punkcie.
{: .text-justify}:
![11](/assets/images/hacking/2023/04/11.png)
{: .text-justify}:
# 04. Stenografia
Ściągamy obrazki na dysk i skanujemy je programem **stegseek**
```bash
...
Po diseambleacji w [Ghidrze](https://ghidra-sre.org/) widzimy takie coś.
{: .text-justify}:
![12](/assets/images/hacking/2023/04/12.png)
{: .text-justify}:
Po odkodowaniu mamy ilovecats
```bash
printf '%b%b%b%b%b%b%b%b' "\x69\x6c\x6f\x76\x65\x63\x61\x74\x73\n"
ilovecats
```
## 05b. Łamanie hasła metodą bruteforce
Do tego użyłem **Crackmapexec**. Niestety inne programy tego typu tj: **Hydra**, **Medusa**, **Ncrack** wyrzucały cały czas błędy przy skanowaniu albo nie wiedziałem, czy program dobrze działa, czy nie zawiesił się. Na szczęście **Crackmapexec** był ok.
```bash
...
```
Będąc w katalogu __\operations\New folder\deploy\3\latest\approved\__ należy ściągnąć plik **zurrak.old.vmdk**
{: .text-justify}:
```bash
...
```
# 07. zurrak.old.vmdk
Tę wirtualkę odpaliłem w **WMware WorkStation player 17**. Pamiętaj jednak, że jak masz zainstalowanego **Virtual Box**a to najlepiej na chwilę wyłączyć interfejs sieciowy **virtual Boxa**, jeżeli masz potem zamiar korzystać z sieci,  Jednak pójdziemy na skróty i widząc **Grub**a wciskamy **e** a potem edytujemy linię, tam gdzie jest __linux__, zamieniamy **ro  quit** na **rw init=/bin/bash**. **F10** i jazda. w __/etc/shadows__ są hasła: **root** i **postgres**. A jak ściągnąć cały plik nie przepisując? Wystarczy go przerzucić przez **nc**, ale to zostawiam na ćwiczenie dla czytelnika.
{: .text-justify}:
![13](/assets/images/hacking/2023/04/13.png)
# 08. postgres
Odpalamy znowu naszego **Hashcat**a i działamy. o parametry się nie martw. Hashcat zazwyczaj znajduje poprawnego hasha.
{: .text-justify}:
```bash
.\hashcat.exe -O -a0 .\hashe\zurrak-shadow.txt .\dict\rockyou.txt
```
Po krótkim czasie mamy hasełko:
{: .text-justify}:
![14](/assets/images/hacking/2023/04/14.png)
Mamy login i hasło do **Postgres**a. Wracamy do naszego **Metasploit**a i szukamy modułu **postgres_copy_from_program_cmd_exec**
{: .text-justify}:
```bash
...
[*] Command stager progress: 100.00% (773/773 bytes)
 ...
 Dzięki modułowi **postgres_copy_from_program_cmd_exec** można wejść na konsolę maszyny. Inaczej też możnabyłoby wejść, ale już trzeba byłoby wchodzić bezpośrednio na maszynę.
 {: .text-justify}:
 ```bash
...
```
Login **emre**, hasło **daily666**. Przypomina to **SMB**. W konfiguracji __/etc/samba/smb.conf__ widać takie coś:
{: .text-justify}:
```plaintext
[internal]
...
```
Ten plik **emergency.sh** zawiera zmianę hasła na root, wrzućmy go do __/opt/internal__ logując się przez **Smbclient**
```bash
