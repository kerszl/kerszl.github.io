...
Przechwytując stronę przez **Burp Suit**a widzimy **token**, ale teraz jest on nieistotny. Będąc w tym programie, wrzucamy wszystko do **Repeater**a. Tak jak się spodziewaliśmy, dalej jest tylko początek strony **Welcome**.
{: .text-justify}
...
Należy zwrócić uwagę na to, że pole z adminem wygląda tak: __"isAdmin": false__,. Spróbujmy więc złamać ten token, uzyskać hasło i zmienić __"isAdmin":__ na __true__. Do tego łamania haseł dobrze nadaje się **Hashcat**:
{: .text-justify}
```bash
.\hashcat.exe -O -a0 .\hashe\zurrak.txt .\dict\rockyou.txt
```
![09](/assets/images/hacking/2023/04/09.png)
**Hashcat** automatycznie złamał hasło - **TEST123**.
{: .text-justify}
Wpisujemy hasło, zmieniamy __"isAdmin"__ na __true__
{: .text-justify}
![10](/assets/images/hacking/2023/04/10.png)
{: .text-justify}
Po wrzuceniu tokenu mamy taką samą reakcję, jak we wcześniejszym punkcie.
{: .text-justify}
...
Po diseambleacji w [Ghidrze](https://ghidra-sre.org/) możemy zauważyć.
{: .text-justify}
...
# 08. Postgres
Odpalamy znowu naszego **Hashcat**a i działamy. o parametry się nie martw. Program zazwyczaj znajduje poprawne hashe.
{: .text-justify}
```bash
...
```
Ten skrypt **emergency.sh** zawiera zmianę hasła na root. Wrzućmy go do __/opt/internal__ logując się przez **Smbclient**
{: .text-justify}
```bash
