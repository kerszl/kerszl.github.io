...
## 01. 3..2..1.. start
Na samym wstępie autor prosi, żeby w **DNS** przypisać ip do nazwy hosta. W tym wypadku host __wordy__ dodajemy do __/etc/hosts__.
{: .text-justify}
...
Dzięki temu będziemy mogli się odwoływać bezpośrednio do wirtualki poprzez np. `ping wordy`, a nie przez `ping 172.16.1.214`. Bez tego wpisu w __/etc/hosts__ może nie wszystko poprawnie działać.
{: .text-justify}
...
```
Widzimy, że **graham** może uruchomić komendę __/home/jens/backups.sh__ z konta **jens**. Wyjdźmy z **Metasploit**a i się zalogujmy przez **SSH**, żeby można było włączyć edytor **Nano**. Zedytujmy plik __/home/jens/backups.sh__, żeby wyglądał mniej więcej tak:
{: .text-justify}
```bash
