...
2: eth0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc mq state UP group default qlen 1000
    ...
    inet6 fe80::b046:e6ff:fec8:fb59/64 scope link
       valid_lft forever preferred_lft forever
```
W **IPv6** prefiks zaczynający się od **fe80** oznacza sieć lokalną. Zauważmy, że trzy ostatnie bajty naszej karty sieciowej są tożsame z ostatnimi bajtami **IPv6**. Dzięki tej metodzie zazwyczaj można znaleźć konkretny komputer, który znajduje się w sieci. Jak nie, to trzeba dalej analizować adres MAC. Na lokalnym komputerze wykonajmy poniższą komendę:
{: .text-justify}
```bash
...
Jak widzimy, jest to zwykła strona **Debian**a. Chciałem poffuzować trochę w **IPv6**. Myślałem, że **Feroxbuster** to wspiera. Jednak na razie nie działa to, ale go użyłem, bo ciekawie wyświetla wynik, chociaż **Ffuf** jest chyba szybszy.
{: .text-justify}
...
```
Hasło to **cowboy**, wchodzimy na FTP:
{: .text-justify}
```bash
# root@kali:~# ncftp
...
```
Hasłem do klucza jest **ilovemyself**. Rozpakujmy je i nadajmy odpowiednie uprawnienia:
{: .text-justify}
```bash
...
```
# 07. SSH IPv6
Mamy klucz, rozkodowaliśmy go, żeby nie wpisywać hasła przy logowaniu. Teraz możemy wejść na **Shell**a. Pamiętajcie, żeby na końcu adresu **IPv6** była dopisana sieciówka. W moim wypadku to jest **%eth0**.
{: .text-justify}
```bash
...
# 08. root
Konto **root** to jest już drobnostka, akurat ostatnio w [DC-6](https://kerszl.github.io/hacking/walkthrough/dc-6/) było identyczne rozwiązanie. Opis tej sztuczki innych możesz znaleźć na [gtfobins](https://gtfobins.github.io/). Niestety, po wejściu na **root**a nie widać co piszemy.
{: .text-justify}
```bash
# cromiphi@Hat:~$ sudo -l
...
# 09. Koniec
Z tą wirtualką miałem dosyć dużo problemów
 (IPv6), ale człowiek się cały czas uczy. Jeżeli podobał się wpis, znalazłeś jakiś błąd napisz mejla na [kerszi@protonmail.com](mailto:kerszi@protonmail.com).
{: .text-justify}