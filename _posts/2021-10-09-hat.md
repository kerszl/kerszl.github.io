...
|Poziom:|Średni|
|Nauczysz się:|Ipv6, Kryptografia, Fuzzing|

## 01. Wstęp
**Hat** jest podstępną maszyną, na którą można stracić dużo czasu, ale jak mówił Sherlock Holmes. "Gdy odrzucisz to, co niemożliwe, wszystko pozostałe, choćby najbardziej nieprawdopodobne, musi być prawdą." Te powiedzenie sprawdza się i tutaj. A o co chodzi? Nie będę was trzymał w niepewnoś? Tak, chodzi o **IPv6**. Ile czasu straciłem na to, ale to nie bezowocnie, za to dzięki solucji [Nepcodex](https://nepcodex.com/2021/10/writeup-of-hat-from-hackmyvm-walkthrough/) wszystko stało sie jasne.
{: .text-justify}

## 02. Ustalanie IP maszyny
Najpierw sprawdźmy IP maszyny, jaką ma w **IPv4**. Zapiszmy też sobie adres fizyczny, czyli potocznie **mac address**, to będzie ważne przy ustalaniu adresu w **IPv6**:
{: .text-justify}
```bash
...
...
```
Mamy IPv4 i Mak adres, teraz ustalmy jak to wygląda w **IPv6**. Najpierw sprawdźmy nasz adres IPv6
{: .text-justify}
```bash
...
2: eth0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc mq state UP group default qlen 1000
    ...
    inet6 fe80::b046:e6ff:fec8:fb59/64 scope link
       valid_lft forever preferred_lft forever
```
Prefix zaczynający się od **fe80** oznacza sieć lokalną w **IPv6**. Zauważmy, że trzy ostatnie bajty naszej karty sieciowej są tożsame z **IPv6**. Po tym można znaleźć konkretny serwer, który znajduje się w sieci. Na lokalnym komputerze wykonajmy poniższą komendę:
{: .text-justify}
```bash
...
```
Ale po co do tego używać **IPv6**? Przecież **IPv4** jest łatwiejsze i prostsze w obsłudzę - zapytacie. Zaraz wyjaśnię.
{: .text-justify}
```console
...
msf6 >
```
Jak widzicie **port 22** jest filtrowany. Na początku myślałem, że **Knockd** go blokował, ale to nie było to. To podstępny **IPv6** :smiley: Dochodzi nam jeszcze port 65535
{: .text-justify}

04. WWW i [fe80::f073:39ff:fe62:4e7e]
Port 80 też jest otwarty, wejdźmy na stronę poprzez **IPv6** wpisując ```[fe80::f073:39ff:fe62:4e7e]```
{: .text-justify}
![ipv6](/assets/images/hacking/2021/17/01.jpg))
Jak widzimy, jest to zwykła strona **Debian**a. Chciałem poffuzować trochę w **IPv6**. Myślałem, że **Feroxbuster** to wspiera. Jednak na razie to nie działa z tym. Jednak go użyłem, bo ciekawie wyświetla wynik, chociaż **Ffuf** jest chyba szybszy.
{: .text-justify}
...
```
Użyłem tutaj rozszerzeń **php** i **log**, bo już wcześniej wiedziałem czego mam szukać. :smiley: ```http://172.16.1.166/php-scripts/file.php``` sugeruje nam parametr do **file**. Teraz poffuzujmy **Ffuf**em.
{: .text-justify}
```bash
...
```
Parametrem do **file** jest **6**. Możemy podejrzeć zawartość **/etc/passwd**:
{: .text-justify}
...
Widać użytkownika **cromiphi**. To się nam potem przyda. Za to **vsftpd.log** zawiera:
{: .text-justify}
...
## 06. Pyftpdlib
**ftp_s3cr3t** jest loginem na **FTP** na **65535** porcie. Do odzyskania Hasła niestety trzeba użyć metody **Brute-force**.
{: .text-justify}
...
```
Sekretny klucz jest zakodowany. Użyjmy **Ssh2john** i **hashcat.exe** (Windows):
```bash
...
```
Hasło do klucza **ilovemyself**. Rozpakujmy je i nadajmy odpowiednie uprawnienia:
{: .text-justify}
```bash
...
```
# 07. SSH IPv6
Mamy klucz, rozkodowaliśmy go, żeby nie wpisywać hasła. Teraz możemy wejść na Shella. Pamiętajcie, żeby na końcu adresu była dopisana sieciówka. W moim wypadku to **%eth0**.
{: .text-justify}
```bash
...
# 08. root
Root to jest już drobnostka, akurat ostatnio w [DC-6](https://kerszl.github.io/hacking/walkthrough/dc-6/) było identyczne rozwiązanie. Opis tej sztuczki innych możesz znaleźć na [gtfobins](https://gtfobins.github.io/). Niestety, po wejściu na **root**a nie widać co piszemy.
{: .text-justify}
```bash
# cromiphi@Hat:~$ sudo -l
