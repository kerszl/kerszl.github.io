...
```
__id_rsa__ jest zakodowany. Użyjmy komendy __ssh2john__ i __hashcat.exe__ (Windows), żeby go rozkodować:
{: .text-justify}
```bash
...
```
Hasłem do klucza jest **ilovemyself**. Usuńmy je i nadajmy odpowiednie uprawnienia dla __id_rsa.out__:
{: .text-justify}
```bash
...
```
# 07. SSH IPv6
Klucz usunęliśmy po to, żeby nie wpisywać hasła przy logowaniu. Teraz możemy od razu wejść na **Shell**a. Pamiętajcie, żeby na końcu adresu **IPv6** była dopisana sieciówka. W moim wypadku to jest __%eth0__.
{: .text-justify}
```bash
