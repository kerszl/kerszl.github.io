...
gallery3_4:    
  - url: /assets/images/hacking/2021/12/03.jpg
    image_path: /assets/images/hacking/2021/12/03.jpg
  - url: /assets/images/hacking/2021/12/04.jpg
    image_path: /assets/images/hacking/2021/12/04.jpg
---
# Nowords by sml
You can get it from [here](https://hackmyvm.eu/machines/machine.php?vm=Nowords)
...
```
O is similar to o, so let's drop the o checking, then we'll fix it manually, and later let's drop the "/" from the beginning of the line. We'll see something like this:
{: .text-justify}
```bash
...
We found the **oiufdsnrewjhuiyfsd** file, It's a JPG file:
{: .text-justify}
<div class="notice--primary" markdown="1">
oiufdsnrewjhuiyfsd
...
{: .text-justify}
```bash
# gocr -i oiufdsnrewjhuiyfsd.jpg  -C 'a-zA-Z' word
```
Here we see names, i.e. logins, possibly passwords. On the main page in the source, there was a hint on the main page that usernames and passwords contain only lowercase characters. Let's go over it all:
{: .text-justify}
<div class="notice--primary" markdown="1">
hint
```text
...
```
## 05. Hydra
Let's check if the **login** and **password** match to connect to FTP.
{: .text-justify}
```bash
# hydra -t 16 -V -L loginpass.txt -P loginpass.txt -u ftp://172.16.1.114 -I -o pass4ftp.txt
```
Hydra found login and pass.
## 06. FTP again
By accessing ftpa, we have access to the entire disk. but we have two interesting files in our directory.
{: .text-justify}
```console
-rw-------    1 1000     1000         2114 Sep 27 20:29 command.jpg
-rw-rw-r--    1 1000     1000           42 Sep 28 21:46 log.txt
```
You may notice that the date of the log.txt file changes every minute.
{: .text-justify}
Entering to the directory **/home/me** we find the cause of this confusion :smiley:
{: .text-justify}
```console
ncftp /home/me > ls -la
...
if __name__ == '__main__':
    ...
    os.system(a[0])
```
The **doit.py** script reads the **command.jpg** image and converts it to text. A word of caution, I recommend using the **Consolas** fonts. Other fonts can be problematic in recognizing characters. Keep in mind, that **command.jpg** is real **PNG** image not **JPEG**. At first, I wanted to type the entire command into a picture, but there was a problem with letter recognition. So I typed a short command and additionally uploaded the script.sh to the FTP server.
{: .text-justify}
{% include gallery id="gallery3_4" %}
``cosole
