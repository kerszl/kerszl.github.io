...
```
To wystarczy, więcej nie trzeba skanować. W **index.html** jest zwykła strona **Debiana**. W **info.php** jest informacja o serwerze i PHP i przy dowiadujemy się, że strona stoi na kontenerze LXD:
{: .text-justify}
...
```
# Kontenery
Należy pamiętać, że jest to kontener **LXD **i nie możemy wyjść z niego, przeglądarkę możemy zamknąć. Sesja powinna być utrzymana:
{: .text-justify}
## host1
```console
meterpreter > getuid
...
mike@host2:~$
```
No i tutaj niestety utknąłem. Jak rozwiąże, albo gdzieś znajdę dalsze rozwiązanie dam znać.
Nie myślcie tylko, że wszystko raz dwa zgadywałem i od razu wiedziałem. Tak to tylko wygląda w solucji.
{: .text-justify}
