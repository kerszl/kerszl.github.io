...
---
This time the article is in Polish language. You can use Google to translate it.
# Beloved by cromiphi
Release date: 29.06.2021
Możesz ściągnąć [stąd](https://hackmyvm.eu/machines/machine.php?vm=Beloved)

Coraz ciekawsze te wirtualki. :smiley: Wybiorę byle którą z **HackMyVm** albo z **VulNhub**a i myślę, że niczym mnie nie zaskoczy, a jednak.  Ta maszynka nie ma za dużo zadań, mało ich a dobre. Nauczysz się w niej pewnej podatności plugina **Wordpressa** oraz na końcu zobaczysz siłę gwiazdek i kresek w **Bash**u.
{: .text-justify}
## 01. Skanowanie
...
```
Na serwerze jest **Wordpress**. Od razu wspomnę, że akurat w tej wirtualce nie ma przed nami ukrytych plików i nie ma co szukać.
{: .text-justify}
## 03. Wpscan i ten podatny plugin
Zajmiemy się za to najlepszym programem (na obecną chwilę) do skanowania **Wordpressa**, czyli **Wpscanem** (oryginalnie **Wpscan**). Jednak wcześniej proponuję na stronie **Wpcana** założyć darmowy token, dzięki któremu uzyskamy więcej informacji ze skanowania. Poniższy przykład jest z tokenem, ale bez niego też powinno działać. Chociaż, być może nie wszystkie informacje będą widoczne. Dodam od siebie, że te skanowanie jest dosyć agresywne, może potrwać trochę czasu i nie polecam tego prawdziwych stronach internetowych. Można za to wyłapać bana. Zaczynamy:
{: .text-justify}
```bash
...
{: .text-justify}
## 04 Metasploit i jego cudowne moduły
```console
...
```
## 06. Shell i co dalej?
Wszysto działa, jesteśmy na konsoli, ale co dalej? Z wcześniejszego rozeznania widać, że jesteśmy na **www-data**, ale w systemie prawdopodobnie są **prawdziwe** konta. Sprawdźmy to:
{: .text-justify}
```bash
...
```
Użytkownik **beloved** ma dostęp do pliku **/usr/local/bin/nokogiri**. Od razu powiem, że program jest on napisany w **Ruby**.
{: .text-justify}
```bash
...
```
08. Jest coś jeszcze?
Tak, możemy zobaczyć co jest na tym **Worpress**ie. Dodam tylko, że hasła są widoczne w plikach, a w bazie danych po prostu wklejamy nasze hasło, które sobie stworzymy. Ale to już zostawiam czytelnikowi.
 {: .text-justify}
